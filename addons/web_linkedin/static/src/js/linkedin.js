odoo.define('web_linkedin', function (require) {
    ...
    */
    LinkedinTester = Class.extend({
        ...
        test_linkedin: function() {
            ...
            return this.test_api_key().then(function() {
                ...
                tag.innerHTML = 'api_key : ' + self.api_key + '\nauthorize : true\nscope: r_basicprofile r_emailaddress'; // r_contactinfo r_fullprofile r_emailaddress';
        ...
        test_api_key: function() {
            ...
            return new Model("ir.config_parameter").call("get_param", ["web.linkedin.apikey"]).then(function(a) {
                if (!!a) {
                    self.api_key = a;
                    return true;
                } else {
    ...
    tester = new LinkedinTester();
    
    FieldLinkedin = core.form_widget_registry.get('char').extend({
        template: 'FieldChar.linkedin',
        init: function() {
        ...
        search_linkedin: function() {
            ...
            if (!this.open_in_process) {
                ...
                this.display_dm.add(tester.test_linkedin()).done(function() {
                    self.open_in_process = false;
                    var text = (self.get("value") || "").replace(/^\s+|\s+$/g, "").replace(/\s+/g, " ");
                    tester.test_authentication().done(function() {
        ...
        create_or_modify_company: function (entity) {
            ...
            IN.API.PeopleSearch().fields(commonPeopleFields).params({
                    ...
                    console.debug("Linkedin People Search API Call Error:", er);
                    //children_def.reject();
                    //children_def.resolve();
                });
            
            return $.when(image_def).then(function () {
                return to_change;
            });
        },
    ...
    core.form_widget_registry.add('linkedin', FieldLinkedin);
    
    FieldLinkedinURL = core.form_widget_registry.get('url').extend({
        template: "FieldChar.linkedin_url"
    });

    core.form_widget_registry.add('linkedin_url', FieldLinkedinURL);
    ...
    LinkedinSearchPopup = Dialog.extend({
        ...
        do_search: function(url) {
            ...
            var self = this;
            var deferrers = [];
            this.$el.find(".oe_linkedin_pop_c, .oe_linkedin_pop_p").empty();
        ...
        do_result_companies: function(companies) {
            ...
            return this.display_result(lst, this.$el.find(".oe_linkedin_pop_c"));
        },
        do_result_people: function(people) {
            ...
            return this.display_result(plst, this.$el.find(".oe_linkedin_pop_p"));
        },
        display_result: function(result, $elem) {
            ...
            _.each(result, function(el) {
                var pc = new LinkedinEntityWidget(self, el);
    ...
    LinkedinEntityWidget = Widget.extend({
        ...
        start: function() {
            ...
            if (this.data.__type === "company") {
                this.$el.find("h3").text(this.data.name);
                self.$el.find("img").attr("src", this.data.logoUrl);
                self.$el.find(".oe_linkedin_entity_headline").text(this.data.industry);
            } else { // people
                this.$el.find("h3").text(this.data.formattedName);
                self.$el.find("img").attr("src", this.data.pictureUrl);
                self.$el.find(".oe_linkedin_entity_headline").text(this.data.headline);
            }
        },
    });
});
