...
class aek_redis_store(osv.osv):
    _name = 'aek.redis.store'
    _description = 'Redis Store'
    
    def _register_hook(self, cr):
        cr.execute("SELECT * FROM aek_redis_store ORDER BY id")
        ...
        for serv in result:
            ...
            cr.execute("SELECT id, name FROM aek_redis_channel where redis_conn = %s ORDER BY id" % serv['id'])
    ...
    _columns = {
        ...
        'value_ids': fields.one2many('aek.redis.value', 'redis_conn', string='Keys-Values'),
        'channel_ids': fields.one2many('aek.redis.channel', 'redis_conn', string='Store Channel'),
    }
...
class aek_redis_value(osv.osv):
    _name = 'aek.redis.value'
    _description = 'Redis Value'
    
    def _get_real_value(self, cr, uid, ids, name, args, context):
        res = {}
        for serv in self.browse(cr, uid, ids, context):
            redis_conn = self.pool.get('aek.redis.store').conn(cr, uid, serv.redis_conn.id, context)
            if redis_conn:
    ...
    _columns = {
        ...
        'redis_conn': fields.many2one('aek.redis.store', string="Redis Store", required=True),
    }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context.get('install_mode', False):
            ...
            if tools.config.get('zato_deploy', False):
                deploy = True
        if deploy:
            redis_conn = self.pool.get('aek.redis.store').conn(cr, uid, vals['redis_conn'], context)
            if redis_conn:
    ...
    def unlink(self, cr, uid, ids, context=None):
        for serv in self.read(cr, uid, ids, ['name', 'redis_conn'], context):
            redis_conn = self.pool.get('aek.redis.store').conn(cr, uid, serv['redis_conn'], context)
            if redis_conn:
...
class aek_redis_channel(osv.osv):
    _name = 'aek.redis.channel'
    _description = 'Redis Channel'
    
    _columns = {
        'name': fields.char('Channel Key', size=128, required=True),
        'subscribers': fields.one2many('aek.redis.channel.subscriber', 'channel_id', string='Subscribers'),
        'redis_conn': fields.many2one('aek.redis.store', string="Redis Store", required=True),
    }
    
aek_redis_channel()

class aek_redis_channel_subscriber(osv.osv):
    _name = 'aek.redis.channel.subscriber'
    ...
    _columns = {
        ...
        'channel_id': fields.many2one('aek.redis.channel', string="Redis Channel", required=True),
    }
    
aek_redis_channel_subscriber()
